...
class EntitlementRegistryService {
   ...
   */
  private registerAppAccessEntitlements(): void {
    const appEntitlements: EntitlementDefinition[] = [
      {
        id: 'app:pos',
   ...
   */
  private register(entitlement: EntitlementDefinition): void {
  ...
  getStats() {
    return {
      ...
      byCategory: Object.fromEntries(
        Array.from(this.registry.byCategory.entries()).map(([category, list]) => [
          category,
          list.length,
        ])
      ),
      byResourceType: Object.fromEntries(
        Array.from(this.registry.byResourceType.entries()).map(([resourceType, list]) => [
          resourceType,
          list.length,
        ])
      ),
