...
export interface OrderInput {
  orderId: string;
  orderDate: Date;
  orderAmountNet: number; // Netto-Betrag
  productId?: string;
  productName?: string;
  isPlatformProduct: boolean;
  sellerTenantId?: string; // null = Platform
  sellerTenantName?: string;
  customerUserId: string;
  customerBillingCountry: string; // ISO 3166-1 Alpha-2
}
...
export class CommissionCalculator {
   ...
   */
  async calculateCommissions(order: OrderInput): Promise<CommissionResult> {
   ...
   */
  private async calculateRegionalCommission(
    order: OrderInput, 
    netAmount: number
  ): Promise<CalculatedCommission | null> {
   ...
   */
  private async calculateAffiliateCommission(
    order: OrderInput,
    netAmount: number
  ): Promise<CalculatedCommission | null> {
   ...
   */
  private calculatePlatformFee(_order: OrderInput, netAmount: number): CalculatedCommission {
   ...
   */
  async saveCommissions(result: CommissionResult, order: OrderInput): Promise<void> {
   ...
   */
  async processOrder(order: OrderInput): Promise<CommissionResult> {
    ...
    if (result.commissions.length > 0) {
      ...
      const affiliateCommission = result.commissions.find(
        c => c.type === 'AFFILIATE_FIRST' || c.type === 'AFFILIATE_RECURRING'
      );
   ...
   */
  async refundOrder(orderId: string, reason: string): Promise<number> {
