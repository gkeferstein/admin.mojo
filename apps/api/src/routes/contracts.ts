...
function generateContractHash(contractType: string, version: string, additionalData?: any): string {
...
function renderContract(contractType: string, variables: Record<string, string> = {}): string {
  const template = CONTRACT_TEMPLATES[contractType as keyof typeof CONTRACT_TEMPLATES];
  if (!template) {
...
export default async function contractsRoutes(fastify: FastifyInstance) {
    ...
    try {
      ...
      return {
        ...
        data: {
          type,
          content,
          hash: generateContractHash(type, CONTRACT_TEMPLATES[type as keyof typeof CONTRACT_TEMPLATES].version),
        },
      };
    } catch (error: any) {
  ...
  fastify.get('/api/contracts/verify/:tenantId/:type', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const signature = await prisma.contractSignature.findFirst({
      where: {
        tenantId,
        contractType: type as 'REGIONAL_DISTRIBUTOR' | 'TENANT_AFFILIATE',
      },
      orderBy: { signedAt: 'desc' },
    });
